...
class Cizacl extends CI_Controller	{
	...
	public function index()	{
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $data['data'] = $data;
	...
	public function sessions()
	{
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $data['data'] = $data;
        $this->template->load('admin', 'cizacl/sessions', $data);
	}
	...
	public function sessions_load_data()
	{
		...
		foreach($query->result() as $row)	{
            ...
            unset($array['logged_in']);
            //print_r($array);exit;
			$data->rows[$i]['id'] = $row->session_id;
			$data->rows[$i]['cell'] = array(
				$this->cizacl_mdl->getUser($array['registration_id']),
				$row->session_id,
				$row->ip_address,
				$row->user_agent,
	...
	public function management()
	{
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $data['data'] = $data;
        $this->template->load('admin', 'cizacl/management', $data);
	}
	...
	public function role_add()
	{
		...
		$data['form'] = array(
			...
			'inherit'	=> array(
				'name'		=> 'inherit_id[]',
				'attributes'=> 'id = "inherit_id[]" class="withClearFunctions select"',
				'options'	=> $roles,
				'selected'	=> ''
			),
			'redirect'	=> array(
				'name'		=> 'redirect',
				'attributes'=> 'id = "redirect" class="withClearFunctions select"',
				'options'	=> $resources,
				'selected'	=> ''
			),
			'description'		=> array(
			...
			'default'		=> array(
				'name'		=> 'default',
				'attributes'=> 'id = "default" class="withClearFunctions select"',
				'options'	=> array(
					0		=> $this->lang->line('no'),
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $this->template->load('dialog', 'cizacl/role_oper', $data);
	}
	...
	public function roles_op()
	{
		if($this->input->post('oper') == 'add')	{
			...
			if ($this->form_validation->run() == false)	{
				...
				if($this->db->insert('cizacl_roles',$addnew))	{
                    ...
                    $this->ion_auth_model->create_group($this->input->post('name',true),$this->input->post('description',true));
		...
		elseif($this->input->post('oper') == 'edit')	{
			...
			if ($this->form_validation->run() == false)	{
				...
				if($this->db->update('cizacl_roles', $update, 'cizacl_role_id = '.$this->input->post('id')))	{
                    $this->ion_auth_model->update_group($this->input->post('id'),$this->input->post('name',true));
		...
		elseif($this->input->post('oper') == 'del')	{
			...
			foreach($id as $value)	{
                ...
                $this->ion_auth_model->delete_group($value);
	...
	public function roles_load_data()
	{
		...
		$i = 0;
		foreach($query->result() as $row)	{
            if($row->cizacl_role_name == 'Guest') continue;
	...
	public function resource_add()
	{
		...
		$data['form'] = array(
			...
			'type'			=> array(
				'name'		=> 'type',
				'attributes'=> 'id = "type" class="withClearFunctions select"',
				'options'	=> array(
			...
			'controller'	=> array(
				'name'		=> 'controller',
				'attributes'=> 'id = "controller" class="withClearFunctions select"',
				'options'	=> $controllers,
				'selected'	=> '0'
			),
			'description'		=> array(
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $this->template->load('dialog', 'cizacl/resource_oper', $data);
	}
	...
	public function rule_add()
	{
		...
		$data['form'] = array(
			...
			'role'	=> array(
				'name'		=> 'cizacl_role_id',
				'attributes'=> 'id = "cizacl_role_id" class="withClearFunctions select"',
				'options'	=> $roles,
				'selected'	=> ''
			)
		);
        ...
        $data['title'] = 'Exclusive Private Sale Inc-ACL';
        $this->template->load('dialog', 'cizacl/rule_oper', $data);
	}
	...
	public function rules_load_data()
	{
		...
		foreach($query->result() as $row)	{
			...
			if(!empty($cizacl_rule_cizacl_role_id))	{
				...
				$subquery	= $this->db->get('cizacl_roles');
				$subrow		= $subquery->row();
                if($subrow->cizacl_role_name == 'Guest') continue;
			}
			else	{
