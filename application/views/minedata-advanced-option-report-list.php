...
<!-- Main content -->
    <section role="main" id="main" >
        <hgroup id="main-title" class="thin" style="text-align:left;padding-bottom:0px;">
            ...
            <?php
            //$value=$this -> main_model -> warrant_scarp($event_insert_id_minedata,$dealer_id_upload_data);
            //print_r($value);
            ...
            //$event_insert_id_minedata=420;
            $lead_mining_presets='';
            $lead_mining_presets=$this -> main_model -> get_lead_mining_presets($event_insert_id_minedata);
            $lead_mining_presets_name_get=$this -> settings_model -> get_lead_type_name($lead_mining_presets);
            ?>
            <form action="<?=base_url()?>minedata/mailoutoption/<?php echo $dealerid_get?>" method="post" style="margin-top: 12px;" class="leadlist">
                ...
                <?php
                $total_count_show=0;
                if($lead_mining_presets_name_get!=''){
                $customer_leadlist_check=$this -> main_model -> get_total_count_advance_options_mine_data($event_insert_id_minedata,$dealer_id_upload_data,$lead_group='');
            ...
            <input  type="hidden" id="event_id" value="<?php echo $event_insert_id_minedata?>"/>
            <input type="hidden" name="lead_mining_presets" value="<?php echo $lead_mining_presets_name_get?>" id="lead_mining_presets_select"/>
        </hgroup>
        ...
        <!--Group 1-->
        <?php
        $group_1_details=$this -> settings_model -> getgroupname_advanced_option($event_insert_id_minedata,1);
        if(isset($group_1_details) && $group_1_details!=''){
            $commoncheck=0;
            foreach($group_1_details as $value_group1){
                $group_name_1=$this -> settings_model -> getreporttype($value_group1['report_type']); 
                $customer_data=$this -> settings_model -> get_advanced_option_group_details_mine_data($event_insert_id_minedata,1,$dealer_id_upload_data,$leadlist_id); 
                if(!empty($customer_data)){
                    $count_group_1=count($customer_data);
                    $check_all='checked';
                }else{
                    $count_group_1='0';
                } 
                ?>
                <div class="slidingbackground"><div class="checkboxalignclass"><input type="checkbox" name="equity_scrap" id="equity_check-all-pr1" class="check-all-pr1" value="1" style="margin-top:7px;"<?php echo $check_all?>  onclick="togglecheckboxes(this,'checkedpr1[]')" ></div><div style="width: 83%;"><a href="javascript:void(0)" class="show_hide " >Report Group 1: <?php echo $group_name_1;?><span class="ui-icon ui-icon-triangle-1-e ui-iconsv" style=" margin-top: -1px;"></span></a></div><div class="leadstext">Leads : <label id="showcountlable"><?php echo $count_group_1?></label></div></div>
                            ...
                            foreach($customer_data as $key=>$value){
                                ...
                                if($value['buyer_homephone']!=''){
                                    preg_match('/\(([^\)]*)\)/', $value['buyer_homephone'], $matches);
                                      //if the phone number is already formatted
                                     if(!empty($matches)){
...
?>
        ...
        $check_all='';
        $count_group_2='0';
        $group_2_details=$this -> settings_model -> getgroupname_advanced_option($event_insert_id_minedata,2);
        if(isset($group_2_details) && $group_2_details!=''){
            foreach($group_2_details as $value_group1){
                $group_name_2=$this -> settings_model -> getreporttype($value_group1['report_type']); 
                $customer_data=$this -> settings_model -> get_advanced_option_group_details_mine_data($event_insert_id_minedata,2,$dealer_id_upload_data,$leadlist_id);
                  if(!empty($customer_data))
                    {
                        $count_group_2=count($customer_data);
                        $check_all='checked';
                    }
                    else
                    {
    ...
    ?>
        ...
        $check_all='';
        $count_group_3='0';
        $group_3_details=$this -> settings_model -> getgroupname_advanced_option($event_insert_id_minedata,3);
        if(isset($group_3_details) && $group_3_details!=''){
            foreach($group_3_details as $value_group1){
                $group_name_3=$this -> settings_model -> getreporttype($value_group1['report_type']); 
                $customer_data=$this -> settings_model -> get_advanced_option_group_details_mine_data($event_insert_id_minedata,3,$dealer_id_upload_data,$leadlist_id);
                    if(!empty($customer_data)){
                        $count_group_3=count($customer_data);
                        $check_all='checked';
                    }else{
                        $count_group_3='0';
                    }
    ...
    $check_all='';
    $count_group_4='0';
    $group_4_details=$this -> settings_model -> getgroupname_advanced_option($event_insert_id_minedata,4);
    if(isset($group_4_details) && $group_4_details!=''){
        foreach($group_4_details as $value_group1){
            $group_name_4=$this -> settings_model -> getreporttype($value_group1['report_type']); 
            $customer_data=$this -> settings_model -> get_advanced_option_group_details_mine_data($event_insert_id_minedata,4,$dealer_id_upload_data,$leadlist_id);
            if(!empty($customer_data)){
                $count_group_4=count($customer_data);
                $check_all='checked';
            }else{
                $count_group_4='0';
            }
            ?>
            <div class="slidingbackground" style="margin-top:1px;"><div class="checkboxalignclass"><input type="checkbox" name="wrranty_scrap" id="warraent-check-all-pr1" class="check-all-pr1" value="4"  style="margin-top:9px;" <?php echo $check_all?> onclick="togglecheckboxes_warrantyscrape(this,'checkedprwarrantyscrape[]')" ></div><div style="width: 83%;"><a href="javascript:void(0)" class="show_hide3">Report Group 4: <?php echo $group_name_4;?><span class="ui-icon ui-icon-triangle-1-e ui-iconsv3" style=" margin-top: -1px;"></span></a></div><div class="leadstext">Leads : <label id="showcountlable_warrantyscrape"><?php echo $count_group_4?></label></div></div>
    ...
    $check_all='';
    $count_group_5='0';
    $group_5_details=$this -> settings_model -> getgroupname_advanced_option($event_insert_id_minedata,5);
    if(isset($group_5_details) && $group_5_details!=''){
        foreach($group_5_details as $value_group1){
            $group_name_5=$this -> settings_model -> getreporttype($value_group1['report_type']); 
            $customer_data=$this -> settings_model -> get_advanced_option_group_details_mine_data($event_insert_id_minedata,5,$dealer_id_upload_data,$leadlist_id);
            if(!empty($customer_data)){
                $count_group_5=count($customer_data);
                $check_all='checked';
            }else{
                $count_group_5='0';
            }
            ?>
            <div class="slidingbackground" style="margin-top:1px;"><div class="checkboxalignclass"><input type="checkbox" name="custom_campain" id="custom-check-all-pr1" class="check-all-pr1" style="margin-top:9px;" value="5" <?php echo $check_all?> onclick="togglecheckboxes_customcampaign(this,'checkedpcustomcampaign[]')" ></div><div style="width: 83%;"><a href="javascript:void(0)" class="show_hide4">Report Group 5: <?php echo $group_name_5;?><span class="ui-icon ui-icon-triangle-1-e ui-iconsv4" style=" margin-top: -1px;"></span></a></div><div class="leadstext">Leads : <label id="showcountlable_customcampaign"><?php echo $count_group_5?></label></div></div>
    ...
    ?>
        <!--Group-5-->
            ...
            <p class="wrapped left-icon icon-info-round" style="text-align: left;">
                ...
                <span style="float: right;" class="total_leads">Total Leads : <label id="totalleadget1"><?=$total_count_show?></label></span>
            </p>
        </form>
    </div>
</section>
