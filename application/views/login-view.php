...
<head>
	...
	<title>Exclusive Private Sale Inc-Login</title>
	...
	<!-- For Retina displays -->
	<link rel="stylesheet" media="only all and (-webkit-min-device-pixel-ratio: 1.5), only screen and (-o-min-device-pixel-ratio: 3/2), only screen and (min-device-pixel-ratio: 1.5)" href="<?=base_url()?>css/2x.css?v=1">
	<!-- Additional styles -->
                ...
                <!--login form starts here-->
					<form method="post" action="" id="form-login" class="input-wrapper blue-gradient glossy" title="Login">
						<ul class="inputs black-input large">
							...
							<li><span class="icon-user mid-margin-right"></span><input type="text" name="login" id="login" value="" class="input-unstyled" placeholder="Username " autocomplete="off"></li>
		...
		$(document).ready(function()
		{
			...
			$('#form-login').submit(function(event)
			{
				...
				else
				{
                   ...
                   $.ajax({
                  url: "<?php echo base_url()?>login/loginprocess",
                  data: 'mail=' + login + '&password=' + encodeURIComponent(pass),
                  type: "POST",
                  success: function(data){
            ...
            if(data=='Done')
            {
                self.parent.location.href='<?php echo base_url()?>login/loginredirect';
                }
            else
            {
