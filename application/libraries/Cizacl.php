...
class CI_Cizacl
{
	...
	function __construct()
	{
		...
		foreach($qRules->result() as $rules)	{
			...
			if(!empty($rules->cizacl_rule_cizacl_resource_function))	{
                $privileges_json = json_decode($rules->cizacl_rule_cizacl_resource_function);
                if (!empty($privileges_json)) {
                    foreach($privileges_json as $privilege)	{
                        if(!empty($privilege))
	...
	function check_cizacl()
	{
		...
		/*echo '<pre>Role'.$role.'</pre>';
		echo '<pre>Resource'.$resource.'</pre>';
		echo '<pre>Privilege'.$privilege.'</pre>';
        exit;*/
		if(!$this->cizacl->isAllowed($role,$resource,$privilege)){
            $this->session->set_flashdata('error', $this->lang->line('no_permissions').' or your session has expired');
