...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
            ...
            $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('status', 'VERIFIED')->users()->row();  //changed to get_user_by_identity from email
	 ...
	 * @author Mathew
	 **/
	public function register($membership_type, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ion_auth_model->register($membership_type, $password, $email, $additional_data, $group_ids);
			if ($id !== FALSE)
			{
	...
	{
		$user_id = $this->session->userdata('registration_id');
		if (!empty($user_id))
		{
			return $user_id;
		}
    ...
    public function getGroupName($group_name){
        $group = $this->ion_auth_model->getGroupName($group_name)->row();
        return $group;
    }
	/**
	 * in_group
