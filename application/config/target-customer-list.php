...
<script>
    ...
    event_id
        ...
        data: 'equity_scrap=' + equity_scrap + '&model_break_down=' + model_break_down+ '&fuel_effciency=' + fuel_effciency+ '&wrranty_scrap=' + wrranty_scrap+ '&custom_campain=' + custom_campain+ '&event_id=' + event_id+ '&fuel_report6=' + fuel_report6,
        type: "POST",
...
<!-- Main content -->
    <section role="main" id="main" style="margin-left:315px">
        ...
        ?>
            <form action="<?=base_url()?>campaign/mailoutoption/<?=$dealerid_get?>" method="post" style="margin-top: 12px;" class="leadlist">
                ...
                if(isset($leadlist) && $leadlist!=''){
                   ...
                   $customer_leadlist_model_breakdown_count=$this -> settings_model -> getleadcount($event_insert_id,'2');
                    ...
                    $customer_leadlist_fuel_report6_count=$this -> settings_model -> getleadcount($event_insert_id,'6');
                    $total_count_show=$customer_leadlist_equity_scrap_count+$customer_leadlist_model_breakdown_count+$customer_leadlist_fuel_efficiency_count+$customer_leadlist_advance_option_count+$customer_leadlist_warrant_scarp_count+$customer_leadlist_fuel_report6_count;
                }
                else{
                    
                    $customer_leadlist_check=$this -> main_model -> total_leadcount_display($lead_mining_presets,$dealer_id_upload_data);
            ...
            <input type="hidden" name="lead_mining_presets" value="<?php echo $lead_mining_presets?>" />
            </hgroup>
            ...
            if(isset($leadlist) && $leadlist!=''){
                $customer_leadlist_equity_scrap_count_get=$this -> settings_model -> getleadcount($event_insert_id,'1');
                $count_equity_scrap=$customer_leadlist_equity_scrap_count_get;
                    if($count_equity_scrap>0){
                        $check_all='checked';
                    }
                    else
                    {
                   ...
                   if($lead_mining_presets=='equity_scrape')
                    {
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    else if($lead_mining_presets=='model_breakdown'){
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'Car');
                                       
                    }
                    else if($lead_mining_presets=='effiecency'){
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'Car',1);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
            ...
            <?php
                  if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data_warranty_scrap=$this -> main_model -> warrant_scarp($event_insert_id,$dealer_id_upload_data,0,5000,1);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
            ?>
                <div class="slidingbackground"><div class="checkboxalignclass"><input type="checkbox" name="equity_scrap" id="equity_check-all-pr1" class="check-all-pr1" value="1" style="margin-top:7px;"<?php echo $check_all?>  onclick="togglecheckboxes(this,'checkedpr1[]')" ></div><div style="width: 83%;"><a href="javascript:void(0)" class="show_hide " ><?php echo $group_name[0];?><span class="ui-icon ui-icon-triangle-1-e ui-iconsv" style=" margin-top: -1px;"></span></a></div><div class="leadstext">Leads : <label id="showcountlable"><?=$count_equity_scrap?></label></div></div>
                <div class="slidingDiv">
                    <table class="table responsive-table" id="sorting-advanced1">
                        ...
                        <tbody>
                        <?php
                    if($lead_mining_presets=='warranty_scrape'){
                        $buyer_first_name='';
                        $buyer_last_name='';
                        $sold_vehicle_year='';
                        $sold_vehicle_make ='';
                        ...
                        $buyer_city='';
                        $pbs_customer_id='';
                    if(isset($customer_data_warranty_scrap) && is_array($customer_data_warranty_scrap)){
                    $vale_count=1;
                    $i=1;
                    //print_r($customer_data_warranty_scrap);
                    foreach ($customer_data_warranty_scrap as $rows){
                    // print_r($rows);
                    ...
                    if($Key=='contract_date'){
                               ...
                               if($Key=='buyer_first_name'){
                               if($value!=''){
                                $buyer_first_name=ucfirst(strtolower($value));
                                }
                                else{
                                   $buyer_first_name ='N/A';
                                }
                                }
                                if($Key=='buyer_last_name'){
                                if($value!=''){
                                $buyer_last_name=ucfirst(strtolower($value));
                                }
                                else{
                                $buyer_last_name='';    
                                }
                                ...
                                if($Key=='buyer_city'){
                               if($value!=''){
                                 $buyer_city=ucfirst(strtolower($value));
                                }
                                else{
                                   $buyer_city ='N/A';
                                }
                               ...
                               if($Key=='sold_vehicle_year'){
                               if($value!=''){
                                $sold_vehicle_year=ucfirst(strtolower($value));
                                }
                                else{
                                   $sold_vehicle_year ='';
                                }
                                }
                                 if($Key=='sold_vehicle_make'){
                               if($value!=''){
                                $sold_vehicle_make=ucfirst(strtolower($value));
                                }
                                else{
                                   $sold_vehicle_make ='';
                                }
                                }
                                 if($Key=='sold_vehicle_model'){
                               if($value!=''){
                                $sold_vehicle_model=ucfirst(strtolower($value));
                                }
                                else{
                                   $sold_vehicle_model ='';
                                }
                                }
                                if($Key=='buyer_homephone'){
                               if($value!=''){
                                $buyer_homephone_string=$value;
                                 $string1=substr($buyer_homephone_string, 0, 3);
                                 $string2=substr($buyer_homephone_string, 3, -4);
                                 $string3=substr($buyer_homephone_string, 6, 10);
                                ...
                                if($Key=='id'){
                                $pbs_customer_id=$value;
                                }
                                ?>
                                   <tr>
                                    <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedpr1[]" class="check-equity-scrape" id="check-all-pr-value" value="<?php echo $pbs_customer_id?>" style="margin-top:9px;"  onclick="showcount();"></th>
                                    <td style="width:220px;color:#666666;"><?=$buyer_first_name .' '.$buyer_last_name?> </td>
                                    ...
                                    <td class="align-center hide-on-mobile" style="color:#666666;"><?=$sold_vehicle_model;?></td>
                                    ...
                                    <a href="<?=base_url()?>campaign/get_customer_details_with_customer_id/<?php echo $pbs_customer_id?>" class="button compact with-tooltip  fancybox fancybox.iframe" title="View Customer Details">Details</a>
                                    </span>
                                    </td>
                                </tr>
                     <?php         
                    ...
                    if(isset($customer_data) && is_array($customer_data)){
                                ...
                                foreach($customer_data as $key=>$value){
                                    ...
                                    $purchase_date=$this -> main_model -> getpurchaesdate($value['sold_vehicle_stock']);
                                    if($purchase_date!=''){
                                    ...
                                    if(isset($leadlist) && $leadlist!=''){
                                    $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'1');
                                        if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                            foreach($customer_leadlist_check as $leadlist_id){
                                                $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                            }
                                        }
                                        else{
                                            $leadlist_id_get_array[]='';
                                        }
                                    if(!empty($leadlist_id_get_array)){
                                        if(in_array($pbs_customer_id,$leadlist_id_get_array)){
                                            $checked='checked';
                                        }else{
                                            $checked='';
                                        }
                                ...
                                ?>
                                <tr>
                                    <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedpr1[]" class="check-equity-scrape" id="check-all-pr-value" value="<?php echo $pbs_customer_id?>" style="margin-top:9px;"  <?php echo $checked?>  onclick="showcount();"></th>
            ...
            if(isset($leadlist) && $leadlist!=''){
                $count_model_breakdown=$this -> settings_model -> getleadcount($event_insert_id,'2');
                if($count_model_breakdown>0){
                    $check_all='checked';
                }
                else{
                    $check_all='';
            ...
            else
            {
                if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'SUV');
                    }
                    else if($lead_mining_presets=='effiecency'){
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'Car',2);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
            ...
            <table class="table responsive-table" id="sorting-advanced1">
                ...
                <?php
                  if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data_warranty_scrap=$this -> main_model -> warrant_scarp($event_insert_id,$dealer_id_upload_data,0,5000,2);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    ...
                    foreach($customer_data as $key=>$value)
                        {
                            ...
                            $pbs_customer_id=$value['id'];
                            if(isset($leadlist) && $leadlist!=''){
                                $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'2');
                                    if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                        foreach($customer_leadlist_check as $leadlist_id){
                                        $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                        }
                        ...
                        ?>
                        <tr>
                            <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedprmodebreakdown[]" class="check-model-breakdown" id="check-all-pr-value"   value="<?php echo $pbs_customer_id?>" <?php echo $checked?> onclick="showcount_modelbreakdown();"></th>
                            <td style="width:220px;color:#666666;"><?=$buyer_first_name?> </td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="width:220px;color:#666666;"><?=$buyer_city?></td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="color:#666666;"><label><?=$buyer_homephone?></label></td>
                            <td class="align-center hide-on-mobile" style="color:#666666;"><?=$buyer_make_model;?></td>
            ...
            if(isset($leadlist) && $leadlist!=''){
                $customer_leadlist_fuel_effciency=$this -> settings_model -> getleadcount($event_insert_id,'3');
                $count_leadlist_fuel_effciency=$customer_leadlist_fuel_effciency;
                if($count_leadlist_fuel_effciency>0){
                    $check_all='checked';
                }
                else{
                    $check_all='';
            ...
            else
            {
                if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'Truck');
                    }
                    else if($lead_mining_presets=='effiecency'){
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'SUV',3);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
            ...
            <table class="table responsive-table" id="sorting-advanced1">
                ...
                <?php
                 if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data_warranty_scrap=$this -> main_model -> warrant_scarp($event_insert_id,$dealer_id_upload_data,0,5000,3);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    ...
                    if(isset($customer_data) && is_array($customer_data)){
                        foreach($customer_data as $key=>$value){
                            ...
                            $pbs_customer_id=$value['id'];
                                if(isset($leadlist) && $leadlist!=''){
                                    $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'3');
                                        if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                            foreach($customer_leadlist_check as $leadlist_id){
                                                $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                            }
                            ...
                            ?>
                            <tr>
                                <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedpr_fuelefficiency[]" class="check-fuel-efficiency"  id="check-all-pr-value" value="<?php echo $pbs_customer_id?>" <?php echo $checked?> onclick="showcount_fuelefficiency()"></th>
                                <td style="width:220px;color:#666666;"><?=$buyer_first_name?> </td>
                                <td class="checkbox-cell" class="align-center hide-on-mobile" style="width:220px;color:#666666;"><?=$buyer_city?></td>
                                <td class="checkbox-cell" class="align-center hide-on-mobile" style="color:#666666;"><label><?=$buyer_homephone?></label></td>
                                <td class="align-center hide-on-mobile" style="color:#666666;"><?=$buyer_make_model;?></td>
            ...
            $count_leadlist_warrenty_scrap='0';
                if(isset($leadlist) && $leadlist!=''){
                $customer_leadlist_warrenty_scrap=$this -> settings_model -> getleadcount($event_insert_id,'4');
                $count_leadlist_warrenty_scrap=$customer_leadlist_warrenty_scrap;
                    if($count_leadlist_warrenty_scrap>0){
                        $check_all='checked';
                    }
                    else{
                        $check_all='';
                ...
                else
                {
                    ...
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'Van');
                    }
                    else if($lead_mining_presets=='effiecency'){
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'SUV',4);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                ...
                <div class="slidingDiv3">
                   ...
                   if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'Van');
                     ...
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'Truck',4);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data_warranty_scrap=$this -> main_model -> warrant_scarp($event_insert_id,$dealer_id_upload_data,0,5000,4);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    ...
                    if(isset($customer_data) && is_array($customer_data))
                        ...
                        foreach($customer_data as $key=>$value)
                            ...
                            $pbs_customer_id=$value['id'];
                                if(isset($leadlist) && $leadlist!=''){
                                    $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'4');
                                    if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                        foreach($customer_leadlist_check as $leadlist_id){
                                        $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                        }
                            ...
                            ?>
                            <tr>
                                <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedprwarrantyscrape[]" class="check-fuel-warranty-scrape" id="check-all-pr-value" value="<?php echo $pbs_customer_id?>"  <?php echo $checked?> onclick="showcount_warrantyscrape();"></th>
                                <td style="width:220px;color:#666666;"><?=$buyer_first_name?> </td>
                                <td class="checkbox-cell" class="align-center hide-on-mobile" style="width:220px;color:#666666;"><?=$buyer_city?></td>
                                <td class="checkbox-cell" class="align-center hide-on-mobile" style="color:#666666;"><label><?=$buyer_homephone?></label></td>
                                <td class="align-center hide-on-mobile" style="color:#666666;"><?=$buyer_make_model;?></td>
            ...
            $count_leadlist_advance_options='0';
                if(isset($leadlist) && $leadlist!=''){
                    $customer_leadlist_advance_options=$this -> settings_model -> getleadcount($event_insert_id,'5');
                    $count_leadlist_advance_options=$customer_leadlist_advance_options;
                    if($count_leadlist_advance_options>0){
                        $check_all='checked';
                    }
                    else{
                        $check_all='';
                ...
                else
                {
                    ...
                    $customer_data=$this -> main_model -> customerdatalist_model_breakdown($dealer_id_upload_data,0,500,'');
                    }
                    else if($lead_mining_presets=='effiecency'){
                     $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'Truck',5);
                    }
                    else if($lead_mining_presets=='warranty_scrape'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
            ...
            <table class="table responsive-table" id="sorting-advanced1">
                ...
                <?php
                  if($lead_mining_presets=='equity_scrape')
                    ...
                    $customer_data_warranty_scrap=$this -> main_model -> warrant_scarp($event_insert_id,$dealer_id_upload_data,0,5000,5);
                    }
                    else if($lead_mining_presets=='custom_campaign'){
                    $customer_data=$this -> main_model -> customerdatalist($dealer_id_upload_data,0,200);
                    }
                    ...
                    foreach($customer_data as $key=>$value)
                        {
                            ...
                            $pbs_customer_id=$value['id'];
                                if(isset($leadlist) && $leadlist!=''){
                                $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'5');
                                    if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                        foreach($customer_leadlist_check as $leadlist_id){
                                            $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                        }
                        ...
                        ?>
                        <tr>
                            <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedpcustomcampaign[]" class="check-custom-campaign" id="check-all-pr-value" value="<?php echo $pbs_customer_id?>" <?php echo $checked?> onclick="showcount_customcampaign();"></th>
                            <td style="width:220px;color:#666666;"><?=$buyer_first_name?> </td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="width:220px;color:#666666;"><?=$buyer_city?></td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="color:#666666;"><label><?=$buyer_homephone?></label></td>
                            <td class="align-center hide-on-mobile" style="color:#666666;"><?=$buyer_make_model;?></td>
          ...
          if(isset($group_name[5])&& $group_name[5]!=''){
             ...
             $customer_data=$this -> main_model -> customerdatalist_fuel_type($dealer_id_upload_data,0,50,'Truck',6);
            $count_fuel_effency_report6=count($customer_data);
             if(isset($leadlist) && $leadlist!=''){
                    $customer_leadlist_advance_options=$this -> settings_model -> getleadcount($event_insert_id,'6');
                    $count_fuel_effency_report6=$customer_leadlist_advance_options;
                    if($count_fuel_effency_report6>0){
                        $check_all='checked';
                    }
                    else{
                        $check_all='';
                    ...
                    foreach($customer_data as $key=>$value)
                        {
                            ...
                            $pbs_customer_id=$value['id'];
                                if(isset($leadlist) && $leadlist!=''){
                                $customer_leadlist_check=$this -> settings_model -> get_customer_details($event_insert_id,'6');
                                    if(isset($customer_leadlist_check) && is_array($customer_leadlist_check)){
                                        foreach($customer_leadlist_check as $leadlist_id){
                                            $leadlist_id_get_array[]=$leadlist_id['lead_customer_id'];
                                        }
                        ...
                        ?>
                        <tr>
                            <th scope="row" class="checkbox-cell textcolor" style="text-align: center;"><input type="checkbox" name="checkedfuelgroup[]" class="check-fuel-report" id="check-all-pr-value" value="<?php echo $pbs_customer_id?>" <?php echo $checked?> onclick="showcount_fuel_report();"></th>
                            <td style="width:220px;color:#666666;"><?=$buyer_first_name?> </td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="width:220px;color:#666666;"><?=$buyer_city?></td>
                            <td class="checkbox-cell" class="align-center hide-on-mobile" style="color:#666666;"><label><?=$buyer_homephone?></label></td>
                            <td class="align-center hide-on-mobile" style="color:#666666;"><?=$buyer_make_model;?></td>
