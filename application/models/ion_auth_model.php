...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, array('registration_id' => $id));
		}
		else
		{
			$data = array(
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('registration_id, password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		$old_password_matches = $this->hash_password_db($user->registration_id, $old);
	 ...
	 * @author Mathew
	 **/
	public function register($membership_type, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_column == 'email_id' && $this->email_check($email))
		{
		...
		$data = array(
		    ...
		    'created_on' => time(),
		    'status'     => ($manual_activation === false ? 'VERIFIED' : null)
		);
        ...
        if($id){
            if(!in_array($membership_type,array('sub_admin','account_managers'))){
                $user_data = array_merge($this->_filter_data($this->tables[$membership_type], $additional_data), array('registration_id' => $id));
                $this->db->insert($this->tables[$membership_type], $user_data);
            }
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select('registration.'.$this->identity_column . ',  registration.email_id, registration.registration_id, registration.password, registration.status, registration.last_login,registration.usertype,registration.first_name,registration.last_name,registration_groups.group_id as user_cizacl_role_id')
                          ->join('registration_groups','registration_groups.registration_id = registration.registration_id')
		                  ->where('registration.'.$this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('registration.registration_id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->registration_id, $password);
			...
			if ($password === TRUE)
			{
				...
				$this->update_last_login($user->registration_id);

				$this->clear_login_attempts($identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{
					$this->remember_user($user->registration_id);
				}
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.registration_id as registration_id',
			    $this->tables['users'].'.registration_id as user_id'
			));
		}

		//filter by group registration_id(s) if passed
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.registration_id',
				    'inner'
				);
	...
	public function user($id = NULL)
	{
		...
		//if no registration_id was passed use the current users registration_id
		$id || $id = $this->session->userdata('registration_id');

		$this->limit(1);
		$this->order_by('registration_id', 'desc');
		$this->where($this->tables['users'].'.registration_id', $id);
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		$user_id || $user_id = $this->session->userdata('registration_id');
	 ...
	 **/
	public function update($id, array $data, array $additional_data,$membership_type)
	{
		...
		$data = $this->_filter_data($this->tables['users'], array_merge($additional_data,$data));
		...
		$this->trigger_events('extra_where');
		$status = $this->db->update($this->tables['users'], $data, array('registration_id' => $user->registration_id));

        if($status){
            if(!in_array($membership_type,array('sub_admin','account_managers'))){
                $user_data = $this->_filter_data($this->tables[$membership_type], $additional_data);
                $this->db->update($this->tables[$membership_type], $user_data, array('registration_id' => $user->registration_id));
            }
        }
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['users'], array('registration_id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('registration_id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'registration_id'      => $user->registration_id, //everyone likes to overwrite registration_id so we'll use user_id
		    'old_last_login'       => $user->last_login,
		    'logged_in'            => array('registration_id' => $user->registration_id,
            ...
            'user_surname'			=> $user->first_name .' '. $user->first_name,
            'user_lastaccess'		=> $user->last_login,
            'user_cizacl_role_id'	=> $user->user_cizacl_role_id
		);
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('remember_code' => $salt), array('registration_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', registration_id, email_id, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		// return the brand new group registration_id
		return $group_id;
	}
